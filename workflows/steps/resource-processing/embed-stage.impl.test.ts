...
describe('embed-stage timestamp and query handling', () => {
  ...
  it('should document the helper queries that avoid .all()', () => {
    // This test documents that both helper functions in workflows/embed-stage.ts:
    //
    // 1. loadExistingAttachments()
    // 2. deleteExistingFragments()
    //
    // use direct awaits on Drizzle queries instead of calling a non-existent .all() method.
