...
export async function runEmbedStageImpl(input: ProcessResourceInput, structured: StructuredPDFContent) {
  ...
  const attachmentSummaries = attachmentRecords.map((att) => ({
    id: att.id,
    description: att.description ?? null,
    detectedType: att.detectedType ?? null,
    caption: att.caption ?? null,
    ocrText: att.ocrText ?? null,
    isRelevant:
      typeof att.isRelevant === 'number'
        ? att.isRelevant === 1
        : undefined,
  }));
  const attachmentMap = new Map(attachmentRecords.map((att) => [att.id, att]));
  const attachmentSummaryMap = new Map(attachmentSummaries.map((summary) => [summary.id, summary]));
  ...
  const relevantImages = structured.pages.flatMap((page) =>
    page.images
      .filter((img) => {
        const hasGoodQuality = img.isGoodQuality === 'good';
        const isRelevant =
          typeof img.isRelevant === 'number'
            ? img.isRelevant === 1
            : true;
        return hasGoodQuality && isRelevant && !!img.url;
      })
      .map((img) => ({
        image: img,
        page: {
  ...
  const imageFragmentsData = relevantImages
    .map(({ image, page }) => {
      ...
      const attachmentSummary = attachmentSummaryMap.get(image.id) ?? {
        id: image.id,
        description: attachment.description ?? null,
        detectedType: attachment.detectedType ?? null,
        caption: attachment.caption ?? null,
        ocrText: attachment.ocrText ?? null,
        isRelevant:
          typeof attachment.isRelevant === 'number'
            ? attachment.isRelevant === 1
            : undefined,
      };
  ...
  const allFragmentsForEmbedding: EmbeddableFragment[] = [
    ...textFragmentsData.map((item) => {
      const images: ImageMetadata[] = item.chunk.images.map((image) => {
        const attachment = attachmentMap.get(image.id);
        const summary = attachmentSummaryMap.get(image.id);
        return buildFragmentImageMetadata({
          id: image.id,
          url: image.url,
          bbox: image.bbox,
          caption: image.caption ?? summary?.caption ?? null,
          description: image.description ?? summary?.description ?? null,
          detectedType: image.detectedType ?? summary?.detectedType ?? null,
          ocrText: image.ocrText ?? summary?.ocrText ?? null,
          isRelevant:
            typeof image.isRelevant === 'boolean'
              ? image.isRelevant
              : summary?.isRelevant,
        });
      });
    ...
    ...imageFragmentsData.map((item) => {
      const summary = item.attachmentSummary ?? attachmentSummaryMap.get(item.image.id);
      const imageMetadata = item.image.url
        ? buildFragmentImageMetadata({
            id: item.image.id,
            url: item.image.url,
            bbox: item.image.bbox,
            caption: item.image.caption ?? summary?.caption ?? null,
            description: item.image.description ?? summary?.description ?? null,
            detectedType: item.image.detectedType ?? summary?.detectedType ?? null,
            ocrText: item.image.ocrText ?? summary?.ocrText ?? null,
            isRelevant: summary?.isRelevant,
          })
        : null;
...
function buildFragmentImageMetadata(image: {
  ...
  if (image.detectedType) {
    metadata.detectedType = image.detectedType;
  }
  if (image.ocrText) {
    metadata.ocrText = image.ocrText;
  }
  if (typeof image.isRelevant === 'boolean') {
    metadata.isRelevant = image.isRelevant;
  }
