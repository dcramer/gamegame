...
export function replaceImageReferences(
  ...
  let result = markdown.replace(imageRegex, (fullMatch, alt, url) => {
    ...
    // Remove data URIs (images should be uploaded to Vercel Blob, not embedded)
    if (url.startsWith('data:')) {
      console.warn('Removing embedded data URI from markdown - images should be uploaded to blob storage');
