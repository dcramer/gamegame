...
import { bggGames } from '../db/schema/d1';
...
class BGGRequestQueue {
   ...
   * Only protects within a single Worker instance
   */
   ...
   * Uses KV put() with expiration to create a distributed lock
  ...
  private async enqueueDistributed<T>(fn: () => Promise<T>): Promise<T> {
    ...
    while (true) {
      ...
      if (Date.now() - startWait > this.MAX_WAIT) {
        console.error(`BGG rate limit: exceeded maximum wait time (${this.MAX_WAIT}ms), falling back to in-memory`);
        return this.enqueueInMemory(fn);
      }

      try {
        // Try to get current lock
        const currentLock = await this.kv!.get(this.KV_KEY);
        ...
        if (!currentLock) {
          ...
          try {
            ...
            // Lock will auto-expire after 5 seconds
          }
        }
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        ...
        console.log(`${operationName} failed (attempt ${attempt + 1}/${maxRetries + 1}), retrying in ${delay}ms:`, lastError.message);
        await new Promise(resolve => setTimeout(resolve, delay));
      }
    }
  }
 ...
 * Note: Unlike Next.js version, we don't convert to WebP since Sharp doesn't work in Workers
 * Images are returned as-is from BGG
 */
