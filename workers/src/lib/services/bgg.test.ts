...
describe('BGG Service Integration Tests (with Cloudflare fetchMock)', () => {
  ...
  describe('searchBGGGames', () => {
    ...
    it('should use cached thumbnails from database', async () => {
      const db = getDb(env.DB);

      // Pre-populate cache
      await db.insert(bggGames).values({
        id: '224517',
        name: 'Brass: Birmingham',
        yearPublished: 2018,
        thumbnailUrl: 'https://cached-thumbnail.jpg',
        publishers: JSON.stringify([]),
        designers: JSON.stringify([]),
      });

      // Only mock search, not game details (cache should be used)
      fetchMock
        .get('https://boardgamegeek.com')
        .intercept({ path: /\/xmlapi2\/search/ })
        .reply(200, MOCK_SEARCH_BRASS_XML, {
      ...
      expect(results[0].thumbnailUrl).toBe('https://cached-thumbnail.jpg');
    });

    it('should handle API errors gracefully', async () => {
      // Mock API error for all retry attempts (3 times)
      fetchMock
        .get('https://boardgamegeek.com')
        .intercept({ path: /\/xmlapi2\/search/ })
      ...
      await expect(
        searchBGGGames('Brass', env.DB, null, { fetchThumbnails: false })
  ...
  describe('getBGGGameDetails', () => {
    ...
    it('should cache game details to database', async () => {
      fetchMock
        .get('https://boardgamegeek.com')
        .intercept({ path: /\/xmlapi2\/thing/ })
        .reply(200, MOCK_GAME_DETAILS_BRASS_BIRMINGHAM_XML, {
          headers: { 'Content-Type': 'text/xml' },
        });

      await getBGGGameDetails('224517', env.DB, null);
      ...
      const [cached] = await db
        .select()
        .from(bggGames)
        .where(eq(bggGames.id, '224517'))
        .limit(1);

      expect(cached).toBeDefined();
      expect(cached.name).toBe('Brass: Birmingham');
      expect(cached.yearPublished).toBe(2018);
      expect(JSON.parse(cached.publishers!)).toContain('Roxley');
      expect(JSON.parse(cached.designers!)).toContain('Martin Wallace');
    });
    ...
    it('should return cached data when available', async () => {
      ...
      await db.insert(bggGames).values({
        ...
        maxPlayers: 4,
        playingTime: 120,
        imageUrl: 'https://cached-image.jpg',
        thumbnailUrl: 'https://cached-thumb.jpg',
        publishers: JSON.stringify(['Cached Publisher']),
        designers: JSON.stringify(['Cached Designer']),
      });
      ...
      expect(details.publishers).toContain('Cached Publisher');
    });
    ...
    it('should handle missing optional fields', async () => {
      fetchMock
        .get('https://boardgamegeek.com')
        .intercept({ path: /\/xmlapi2\/thing/ })
        .reply(200, MOCK_MINIMAL_GAME_XML, {
          headers: { 'Content-Type': 'text/xml' },
        });

      const details = await getBGGGameDetails('12345', env.DB, null);

      expect(details.name).toBe('Minimal Game');
      expect(details.yearPublished).toBeNull();
      expect(details.minPlayers).toBeNull();
      expect(details.maxPlayers).toBeNull();
      expect(details.imageUrl).toBeNull();
      expect(details.publishers).toEqual([]);
      expect(details.designers).toEqual([]);
    });

    it('should fix protocol-relative URLs', async () => {
      fetchMock
        .get('https://boardgamegeek.com')
        .intercept({ path: /\/xmlapi2\/thing/ })
        .reply(200, MOCK_GAME_DETAILS_BRASS_BIRMINGHAM_XML, {
      ...
      // XML has // prefix, should be converted to https://
      expect(details.imageUrl).toMatch(/^https:\/\//);
    ...
    it('should handle API errors with retry', async () => {
      ...
      fetchMock
        ...
        .reply(() => {
          attemptCount++;
          if (attemptCount < 3) {
            return { statusCode: 503, data: 'Timeout' };
          }
          return {
            statusCode: 200,
            data: MOCK_GAME_DETAILS_BRASS_BIRMINGHAM_XML,
            headers: { 'Content-Type': 'text/xml' },
          };
        })
        .times(3);
      ...
      expect(details.name).toBe('Brass: Birmingham');
