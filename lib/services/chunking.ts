...
const splitter = RecursiveCharacterTextSplitter.fromLanguage("markdown", {
  chunkSize: 1000,
  chunkOverlap: 100, // Add some overlap to preserve context
});

/**
 * Find the current section hierarchy for a given page and position
 */
...
function getRelevantImages(
  ...
  // For now, include all images from the page
  // In the future, we could use bbox coordinates to match images to specific sections
  return page.images
    .filter((img): img is typeof img & { url: string } => !!img.url) // Only include images that have been stored
    .map((img) => ({
      id: img.id,
      url: img.url,
      bbox: img.bbox,
...
async function chunkPage(
  ...
  if (page.markdown.length < 1500 && pageSections.length <= 1) {
    const section = getCurrentSection(page, allSections, 0);
    const images = getRelevantImages(page, 0, page.markdown.length);
 ...
 * @param structured Structured PDF content with pages, sections, and images
